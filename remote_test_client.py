...
def send_request(api_url, payload, description):
    ...
    try:
        ...
        print(f"Error: Could not connect to server at {api_url}. Please ensure the server is running and accessible.")
        return []
...
def main():
    ...
    polys_point = send_request(api_url, payload_point, "Point Prompt (Center Click)")
    ...
    polys_box = send_request(api_url, payload_box, "Box Prompt (Around Green Rect)")
